...
const WeeklySchedule: React.FC<WeeklyScheduleProps> = ({ data }) => {
  ...
  const getItemStyle = (slotTime: string, item: Schedule) => {
    const slotMinutes = timeToMinutes(slotTime)
  ...
  return (
    <div className='flex h-screen flex-col p-8'>
      <div className='mb-4 flex items-center justify-between'>
        ...
        <Select
          value={interval}
          onValueChange={(value: '30m' | '1h') => setInterval(value)}
        >
        ...
        <div className='grid h-full min-w-[800px] grid-cols-8 gap-px bg-gray-100 dark:bg-[#181818]'>
          ...
          {timeSlots.map((time) => (
            <React.Fragment key={time}>
              ...
              {weekdays.map((day) => {
                const dayScheduleItems = data.filter(
                  (item) =>
                    item.weekday === day &&
                    isWithinTimeSlot(time, item.start_time!, item.end_time!)
                )
                return (
                  <div
                    key={`${day}-${time}`}
